...
.prefix = bash_args[3]
if(is.na(.prefix)) .prefix = paste0("perturb_ATE_", .outcome_yrs, "yrs_"
                                    , gsub("-|[ ]|:|\\.", "_", lubridate::now())
                                    )

.outcome = bash_args[4]
# if(.outcome %nin% c("Y_change", "Y_NegChange", "Y_avg")) stop("Incorrect outcome specified, must be one of Y_change, Y_NegChange, Y_avg (case sensitive)")
...
# Parameters #### 
  ...
  .bcd_filename_3yr <- "bcd_allMS_3monthWindow_3yr_2024_10_07.csv"
  .bcd_filename_2yr <- "bcd_allMS_3monthWindow_2yr_2024_10_07.csv"
  
  .ntz_filename_3yr <- "ntz_allMS_3monthWindow_3yr_2024_10_07.csv"
  .ntz_filename_2yr <- "ntz_allMS_3monthWindow_2yr_2024_10_07.csv"

  # .bcd_filename_3yr <- "bcd_allMS_3monthWindow_3yr_2024_06_05.csv" 
  # .bcd_filename_3yr.old <- "bcd_allMS_3yr_2024_03_07.csv"
  # .bcd_filename_2yr <- "bcd_allMS_3monthWindow_2yr_2024_06_05.csv" 
  # .bcd_filename_2yr.old <- "bcd_allMS_2yr_2024_03_07.csv"
  
  # .ntz_filename_3yr <- "ntz_allMS_3monthWindow_3yr_2024_06_05.csv" 
  # .ntz_filename_3yr.old <- "ntz_allMS_3yr_2024_03_07.csv"
  # .ntz_filename_2yr <- "ntz_allMS_3monthWindow_2yr_2024_06_05.csv" 
  # .ntz_filename_2yr.old <- "ntz_allMS_2yr_2024_03_07.csv"
...
# Data Creation/Import #### 
  ...
  # bcd_3yr.old <- read_csv(here("Data", "created_data", "BCD", .bcd_filename_3yr.old))
  bcd_2yr <- read_csv(here("Data", "created_data", "BCD", .bcd_filename_2yr))
  # bcd_2yr.old <- read_csv(here("Data", "created_data", "BCD", .bcd_filename_2yr.old))
  
  .skip_build <- T  # skips cohort_building.R in DMT script 
  source(here("Code", "driver_scripts", "ntz.R"))
  ntz_3yr <- read_csv(here("Data", "created_data", "NTZ", .ntz_filename_3yr))
  # ntz_3yr.old <- read_csv(here("Data", "created_data", "NTZ", .ntz_filename_3yr.old))
  ntz_2yr <- read_csv(here("Data", "created_data", "NTZ", .ntz_filename_2yr))
  # ntz_2yr.old <- read_csv(here("Data", "created_data", "NTZ", .ntz_filename_2yr.old))
  
  bcd_ntz_3yr <- rbind(bcd_3yr, ntz_3yr)
  bcd_ntz_2yr <- rbind(bcd_2yr, ntz_2yr)
  
  # bcd_ntz_3yr <- rbind(bcd_3yr.old, ntz_3yr.old)
  # bcd_ntz_2yr <- rbind(bcd_2yr.old, ntz_2yr.old)
  ...
  saveRDS(object = list(ATE_full = ATE$ATE # ATE[names(ATE)!="ImputeModel"]
          ...
          , file = here("Results", "BCD_NTZ", "ATE", "Sensitivity", paste0(.prefix, "_ATE_BCD_NTZ.RDS")) 
          )
